...
import { Users, Clock, Coffee, LogOut, AlertTriangle } from 'lucide-react';
...
export function TodayRoster() {
  ...
  // Build a set of no-show staff IDs
  const noShowStaffIds = new Set(noShows?.map(ns => ns.staff_id) || []);
  ...
  const rosterStaff: RosterStaff[] = staff.map((s) => {
    ...
    let status: RosterStatus = 'not_arrived';
    let lateMinutes: number | undefined;

    if (todayAttendance) {
      // Check if they clocked in late
      if (todayAttendance.is_late && todayAttendance.status === 'clocked_in') {
        status = 'late';
        lateMinutes = todayAttendance.late_minutes;
      } else {
        status = todayAttendance.status as RosterStatus;
      }
    } else if (noShowStaffIds.has(s.id)) {
      // No attendance record and marked as no-show
      status = 'no_show';
    }
    ...
    return {
      ...
      shiftStartTime: todayShift?.start_time ?? null,
      status,
      lateMinutes,
    };
      ...
      case 'late':
        return (
          <Badge className="bg-warning text-warning-foreground">
            Late ({lateMinutes ? formatLateMinutes(lateMinutes) : ''})
          </Badge>
        );
      case 'clocked_in':
  ...
  // Summary counts
  const onDuty = rosterStaff.filter((s) => s.status === 'clocked_in' || s.status === 'late').length;
  ...
  const noShowCount = rosterStaff.filter((s) => s.status === 'no_show').length;
  const lateCount = rosterStaff.filter((s) => s.status === 'late').length;
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      {member.shiftStartTime && (
                        <>
                          <span>•</span>
                          <span>{member.shiftStartTime.slice(0, 5)}</span>
                        </>
                      )}
                    </div>
                  </div>

                  {getStatusBadge(member.status, member.lateMinutes)}
                </div>
              ))
            )}
          </div>
