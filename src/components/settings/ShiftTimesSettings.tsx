...
import { useShiftTimesSettings } from '@/hooks/useShiftTimesSettings';
import type { ShiftTimesSettings as ShiftTimesSettingsType } from '@/hooks/useShiftTimesSettings';

export function ShiftTimesSettings() {
  const { settings, isLoading, updateSettings } = useShiftTimesSettings();
  const [shiftTimes, setShiftTimes] = useState<ShiftTimesSettingsType>({
    morningStart: '08:00',
    morningEnd: '15:00',
    eveningStart: '15:00',
    eveningEnd: '22:00',
  ...
  useEffect(() => {
    setShiftTimes(settings);
  }, [settings]);

  const handleChange = (field: keyof ShiftTimesSettingsType, value: string) => {
    setShiftTimes((prev) => ({ ...prev, [field]: value }));
    setIsDirty(true);
  };

  const handleSave = () => {
    updateSettings.mutate(shiftTimes);
    setIsDirty(false);
  };
